    ...
    @foreach($usuarios as $user)
        <tr>
            ...
            <td>
                ...
                <form action="{{ route('usuario.eliminar', $user->id) }}" method="POST" id="eliminar_{{ $user->id }}">
                    @csrf
                    @method('DELETE')
                    <!-- Equivalente a -->
                    <!-- <input type="hidden" name="_method" value="DELETE"> -->
                </form>
                <button onclick="document.getElementById('eliminar_{{ $user->id }}').submit()">Eliminar</button>
            </td>
        </tr>
    @endforeach
</table>
